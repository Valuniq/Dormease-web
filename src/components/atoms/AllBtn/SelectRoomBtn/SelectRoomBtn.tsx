...
const SelectRoomBtn: React.FC<Props & React.HtmlHTMLAttributes<HTMLButtonElement>> = ({
  ...
  return (
    <button
      {...props}
      className={`flex justify-between items-center w-403 h-52 rounded-r-8 rounded-l-50 ${selected ? 'bg-gray-grayscale10' : 'opacity-0 hover:opacity-100 hover:bg-gray-grayscale5 hover:hover-transition active:bg-gray-grayscale10'}`}
    >
